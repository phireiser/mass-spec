...
# ---------------------  Fragment encoder wrapper for both backends  ---------------------
class FragSetEncoderWrapper(nn.Module):
    ...
    def __init__(self, enc_mol: "EncMol", d_latent: int):
        ...
        self.enc_mol = enc_mol
        self.d_latent = d_latent
        # GNN-based encoder for derivation trees
        self.gnn1 = GCNConv(d_latent + 1, 64)  # node features: latent + mass
        self.gnn2 = GCNConv(64, d_latent)
    ...
    def _forward_tree(self, deriv_tree_batch: Optional[List[Optional[GeometricData]]]) -> torch.Tensor:
        ...
        for tree in deriv_tree_batch:
            ...
            # Encode fragment graphs using enc_mol
            frag_zs = self.enc_mol([fg for fg in tree.frag_graphs])  # [n_frags, d_latent]

            # Add mass features if available
            if hasattr(tree, 'masses') and tree.masses.numel() > 0:
                masses = tree.masses.to(device).unsqueeze(-1)  # [n_frags, 1]
                x = torch.cat([frag_zs, masses], dim=-1)  # [n_frags, d_latent+1]
            else:
                # Pad with zeros if no masses
        ...
        return torch.cat(outs, dim=0)  # [B, d_latent]
...
class Sample:
    ...
    # Derivation-tree backend (alternative to adjacency)
    frag_deriv_tree_fwd: Optional[GeometricData] = None  # PyG Data: nodes=fragments, edges from targets (fwd)
    frag_deriv_tree_bwd: Optional[GeometricData] = None  # PyG Data: nodes=fragments, edges from targets (bwd)
...
def build_derivation_tree_from_collection(
    ...
    """
    Build a PyTorch Geometric Data object representing a derivation tree.

    Nodes: store fragment graph indices and masses
    Edges: directed parent->child edges from derivation_graph targets

    Parameters
    ----------
    frag_graphs_list : List[GeometricData]
        Fragment graphs in order (indices correspond to node indices).
    frag_masses : torch.Tensor
        Per-fragment normalized masses [n_frags].
    derivation_graph : List[Tuple[...]]
        List of (frag_smiles, exact_mass, targets, rules).
    frag_smiles_to_idx : Dict[str, int]
        Mapping from fragment SMILES to local index.

    Returns
    -------
    Optional[GeometricData]
        PyG Data with frag_graphs=List[GeometricData], masses=[n_frags], edge_index=[2, n_edges].
    ...
    # Build edge list from derivation graph targets
    edge_list = []
    ...
    if edge_index.numel() > 0:
        assert edge_index.max() < n_frags, f"Edge index out of bounds: max={edge_index.max()}, n_frags={n_frags}"
        assert edge_index.min() >= 0, f"Negative edge index: min={edge_index.min()}"

    # Store fragment graphs and masses as attributes (not standard PyG fields)
...
class RealDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        graph_backend: str = "adjacency",  # "adjacency" or "derivation_tree"
    ):
        ...
        assert self.graph_backend in ("adjacency", "derivation_tree"), \
            f"graph_backend must be 'adjacency' or 'derivation_tree', got {self.graph_backend}"
    ...
    def __getitem__(self, idx: int):
        ...
        # Build derivation trees if requested
        frag_deriv_tree_fwd = None
        frag_deriv_tree_bwd = None
        if self.graph_backend == "derivation_tree" and len(frag_graphs) > 0:
            # Build local index for fragment SMILES
            frag_idx = {s: i for i, s in enumerate(frag_smiles)}
            fwd, bwd = list(self.frag_coll.values())[idx]
            frag_deriv_tree_fwd = build_derivation_tree_from_collection(
                frag_graphs, frag_masses, fwd, frag_idx
            )
            frag_deriv_tree_bwd = build_derivation_tree_from_collection(
                frag_graphs, frag_masses, bwd, frag_idx
            )
...
def collate_vlex(batch: List[Sample]):
    ...
    Also returns derivation trees if available (graph_backend=derivation_tree).
    """
    graph_feats = [b.graph_feat for b in batch]
    true_spectrum = torch.stack([b.true_spectrum for b in batch])
    smiles = [b.smiles for b in batch]
...
def train_epoch_phase_a(
    ...
    enc_mol: EncMol,
    enc_spec: EncSpec,
    frag_set_enc: Union[FragSetEncoderVocabless, "FragSetEncoderWrapper"],
    heads: TaskHeads,
    ...
    alpha_diversity : float
        Weight for diversity loss (encourage varied predictions across batch).
    """
    ...
    for batch_data in loader:
        ...
        z_f = frag_set_enc(frag_graphs, deriv_tree_batch=deriv_trees_fwd)  # [B, d]
        ...
        # Diversity loss: encourage varied predictions across batch
        L_div = 0.0
        if alpha_diversity > 0 and spec_hat_masked.size(0) > 1:
            # Push different samples to have different spectra (minimize correlation)
            spec_norm = F.normalize(spec_hat_masked, dim=-1)  # [B, spec_bins]
            corr_matrix = spec_norm @ spec_norm.mT  # [B, B] - use mT for batch transpose
...
def train_epoch_phase_b(
    ...
    contrastive_temp : float
        Temperature for info_nce contrastive loss.
    alpha_retrieval : float
        Weight for retrieval ranking loss (encourages within-batch retrieval accuracy).
    """
    ...
    for batch_data in loader:
        ...
        # Key: z_bwd (from heads on z_s) should produce spec_hat that, when re-encoded byMOL enc_spec, aligns back to z_s
        ...
        # Retrieval ranking loss: penalize if ground-truth mol is not in top-k
        # Simple version: encourage z_bwd to be closer to z_m than to other z_m's in batch
        # CRITICAL: Normalize embeddings to match test-time cosine similarity!
        L_retr = 0.0
        if alpha_retrieval > 0 and z_m.size(0) > 1:
            z_bwd_norm = F.normalize(z_bwd, dim=-1)
            z_m_norm = F.normalize(z_m, dim=-1)
            sim_matrix = z_bwd_norm @ z_m_norm.T / contrastive_temp  # [B, B]
            # Labels are diagonal (each sample should match its corresponding mol)
        ...
        L_cycle_spec = info_nce(z_s, z_m, T=contrastive_temp)  # spectrum latent should align with mol latent
...
class IndexItem:
    ...
    frag_graphs: Optional[List[GeometricData]] = None
    frag_adj_local: Optional[torch.Tensor] = None
    frag_deriv_tree_fwd: Optional[GeometricData] = None  # Derivation tree (forward)
    frag_deriv_tree_bwd: Optional[GeometricData] = None  # Derivation tree (backward)
...
class LatentIndex:
    ...
    def build(self, dataset: Dataset, device, enc_mol: EncMol):
            ...
            with torch.no_grad():
                for batch_data in loader:
                    # Handle both old and new collate_vlex returns
                    if len(batch_data) == 8:
...
class FaissLatentIndex(LatentIndex):
    ...
    def topk(self, z_query: torch.Tensor, k=10) -> List[IndexItem]:
        ...
        scores, idx = self.index.search(zq, min(k, self.index.ntotal))
        ranked: List[IndexItem] = []
        for j, i in enumerate(idx[0].tolist()):
            it = self.items[i]
            it.score = float(scores[0][j])
            ranked.append(it)
        return ranked
...
def rerank_candidates(spec_q: torch.Tensor,
    ...
    Supports both adjacency and derivation_tree backends.
    """
    ...
    with torch.no_grad():
        ...
        for it in candidates:
            ...
            trees_c = [it.frag_deriv_tree_fwd]  # Use forward tree for reranking
            z_f_c = frag_set_enc([frag_graphs_c], deriv_tree_batch=trees_c)  # [1, d]
...
def infer_mol_to_spec(graph_feat: GeometricData,
    ...
    with torch.no_grad():
        ...
        z_f = frag_set_enc([frag_graphs], deriv_tree_batch=[frag_deriv_tree])  # [1, d]
...
def infer_spec_to_mol(spec: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        _, z_bwd = heads(z_s)  # Apply backward head to match training!
        z_q_n = F.normalize(z_bwd.squeeze(0), dim=-1).cpu()
    cands = index.topk(z_q_n, k=topk)
    if skip_reranking:
        # Return candidates directly from index without reranking
...
def main():
    ...
    p.add_argument("--mol_def_path", type=str, default=Path(__file__).resolve().parents[2] / "data" / "compounds.csv", help="Path to CSV file with molecule definitions (name, SMILES)")
    p.add_argument("--spectra_dir", type=str, default=Path(__file__).resolve().parents[2] / "data" / "nist_spectra", help="Directory containing .jdx spectrum files named by molecule name")
    p.add_argument("--load_path", type=str, default=Path(__file__).resolve().parents[2] / "data" / "processed", help="Directory containing derivation trees")
    args = p.parse_args()
    ...
    # Load Data into mem
    #MOL_DEF_PATH = Path(args.mol_def_path)
