...
However, purly base-data-driven approaches exhibit several limitations:
\begin{itemize}
...
and fragment derivation trees, generated through chemically valid graph transformation rules, 
...
%\subsection{Can graph-based representations improve structure–spectrum alignment?}
%\subsection{Does fragment-awareness improve training efficiency?}
%\subsection{Is interpretability achievable without sacrificing predictive performance?}
...
to generate fragment derivation trees under consistent assumptions. 
...
limits chemical diversity and is does not allow for generalization. 
...
Also to be noted is that the reactions are in terms of graph symbols ambigously defined, and the transformation rules are not exhaustive of all possible fragmentation pathways. 
This means that the generated fragment derivation trees may not capture all relevant chemical transformations, and the model's performance may be sensitive to the specific choice of transformation rules. 
The interpretability analysis is therefore constrained by the assumptions embedded in the rule set, and may not generalize to fragmentation processes that deviate from these assumptions.
