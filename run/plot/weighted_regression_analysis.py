...
print(f"  R² = {r2_w:.6f}")
print(f"  Difference: ΔR² = {abs(r2_w - r2_uw):.6f}")
...
r2_uw = compute_r2_unweighted(ys_f, pred_uw)
print(f"Unweighted: y = {coeffs_uw[2]:.1f} + {coeffs_uw[1]:.1f}*x + {coeffs_uw[0]:.1f}*x²")
print(f"  R² = {r2_uw:.6f}")
...
pred_uw = np.polyval(coeffs_uw, xs_f)
r2_uw = compute_r2_unweighted(ys_f, pred_uw)
print(f"Unweighted: R² = {r2_uw:.6f}")

# Weighted
coeffs_w = np.polyfit(xs_f, ys_f, 3, w=weights_norm)
pred_w = np.polyval(coeffs_w, xs_f)
r2_w = compute_r2_weighted(ys_f, pred_w, weights_norm)
print(f"Weighted:   R² = {r2_w:.6f}")
print(f"  Difference: ΔR² = {abs(r2_w - r2_uw):.6f}")
...
print("\nIf weighted R² is notably different from unweighted, it means the")
