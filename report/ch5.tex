...
The model operates on \emph{precomputed} derivation graphs: candidate fragments and their parent--child relations are generated upstream by the rule-based graph-transformation pipeline and are then encoded by the neural model.
...
Its fragment branch consumes fragment graphs together with their derivation structure --- for example through the derivation-graph backend implemented in \texttt{build\_derivation\_graph\_from\_collection} and \texttt{FragSetEncoderWrapper} --- encoding a graph whose nodes are fragment graphs and whose directed edges represent parent--child relations extracted from the precomputed derivation graph.
...
Crucially, the fragment graph is not treated as fixed input but as a latent object that must itself be predicted: the generative stage is trained to reconstruct an estimated fragmentation directed acyclic graph (DAG), while a second module predicts intensities and hydrogen rearrangements for the generated fragments.
