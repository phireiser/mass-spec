\begin{tikzpicture}[
  ...
  encoderNode/.style  ={rectangle, rounded corners, draw=black, thick, minimum width=2.6cm, minimum height=1.4cm, text centered, align=center, font=\small},
  latentNode/.style   ={circle, draw=black, thick, minimum width=1.5cm, text centered, align=center, font=\small},
  headsNode/.style    ={rectangle, rounded corners, draw=black, thick, minimum width=2.4cm, minimum height=1.2cm, text centered, align=center, font=\small},
  decoderNode/.style  ={rectangle, rounded corners, draw=black, thick, minimum width=2.4cm, minimum height=1.4cm, text centered, align=center, font=\small},
  ...
  outputNode/.style   ={rectangle, rounded corners, draw=black, thick, minimum width=2.4cm, minimum height=1.2cm, text centered, align=center, font=\small},
  arrow/.style        ={->, thick},
...
\node[inputNode, right=of MOL] (FRAG) {Derivation Tree};
\node[inputNode, right=of FRAG] (SPEC) {Mass Spectrum};
\node[groupLabel] at (labelX |- MOL.north) {Input Data};

% ================== ENCODER ROW ==================
\node[encoderNode, below=of MOL]  (EncMol) {EncMol\\GCN $3 \rightarrow 64 \rightarrow d_L$\\Global Pool};
\node[encoderNode, below=of FRAG] (FragSetEnc) {FragTreeEnc\\Fragment Graph +\\Tree GNN + Pool};
\node[encoderNode, below=of SPEC] (EncSpec) {EncSpec \\$s_{bins} \rightarrow 512 \rightarrow d_L$};
...
% ================== LATENT ROW ==================
\node[latentNode, below=of EncMol]     (ZM) {$z_{mol}$};
\node[latentNode, below=of FragSetEnc] (ZF) {$z_{frag}$};
\node[latentNode, below=of EncSpec]    (ZS) {$z_{spec}$};

\node[latentNode, minimum width=1.9cm, below=8mm of ZF] (ZFUSED) {$z_{combined}$};
...
\node[headsNode, below=35mm of ZF] (HdFwd) {Head$_{FWD}$\\$z \to d_t$};
\node[headsNode, below=35mm of ZS]  (HdBwd) {Head$_{BWD}$\\$z \to d_t$};
...
% ================== DECODER ROW ==================
\node[decoderNode, below=of HdFwd] (DecFrag) {DecFrag\\$d_L \to V_{frag}$};
\node[decoderNode, below=of HdBwd] (DecSpec) {DecSpecLatent\\$d_L \to 512 \to s_{bins}$};
\node[groupLabel] at (labelX |- DecFrag.north) {Decoders};

% connect to different anchors so arrows don't overlap
\draw[arrow] (HdFwd) -- (DecFrag);
\draw[arrow] (HdBwd) -- (DecSpec);

% ================== OUTPUT ROW (Spectrum + Fragments only) ==================
\node[outputNode, below=of DecFrag] (FRAG_HAT) {$\hat{f}_{probs}$\\Fragments};
\node[outputNode, below=of DecSpec] (SPEC_HAT) {$\hat{s}_{pred}$\\Spectrum};

\draw[arrow] (DecFrag) -- (FRAG_HAT);
\draw[arrow] (DecSpec) -- (SPEC_HAT);

\end{scope} % end pipeline shift

% ================== INDEX (ALIGNED WITH MOL COLUMN) ==================
\node[outputNode, at=(MOL |- SPEC_HAT)] (INDEX) {Decoder for Retrieval};
