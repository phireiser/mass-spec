...
% Shared x for the loss column (aligned with the ZS-based losses below)
\coordinate (LcolX) at ($(ZS)+(55mm,0mm)$);
\node[lossNode] at ($(LcolX |- SPEC_HAT)+(0,12mm)$) (LOSS_COS)  {Cosine Loss};
\node[lossNode] at (LcolX |- SPEC_HAT)               (LOSS_WASS) {Wasserstein};
%\node[lossNode] at ($(LcolX |- SPEC_HAT)+(0,-12mm)$) (LOSS_FORB) {Forbidden};
\node[lossNode] at (LcolX)                           (LOSS_CON)  {InfoNCE};
