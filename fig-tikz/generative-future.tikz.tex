\begin{tikzpicture}[
    ...
    bigarrow/.style={-{Stealth[length=3.5mm,width=3.2mm]}, line width=1.4pt},
    greenA/.style={bigarrow, genGreen},
    blueA/.style={bigarrow, scoBlue},
    grayA/.style={bigarrow, black!55},
    treearrow/.style={-{Stealth[length=1.6mm]}, dashed, black!60, thin},
    fragnode/.style={circle, draw=genGreen!80, fill=genGreen!50,
                     minimum size=4mm, inner sep=0pt},
    fragempty/.style={circle, draw=genGreen!70, fill=white,
                     minimum size=4mm, inner sep=0pt},
    fraglight/.style={circle, draw=genGreen!60, fill=genGreen!25,
                     minimum size=4mm, inner sep=0pt},
    fragroot/.style={circle, draw=genGreen!80, fill=genGreen!70,
                     minimum size=5mm, inner sep=0pt},
    grayN/.style={circle, draw=nodeGray, fill=nodeGray!55,
...
\definecolor{genGreenL}{RGB}{210,235,220}
\definecolor{genGreenLL}{RGB}{235,245,238}
\definecolor{scoBlue}{RGB}{60,110,200}
\definecolor{scoBlueL}{RGB}{210,225,245}
\definecolor{scoBlueLL}{RGB}{235,243,250}
\definecolor{spectrumBlue}{RGB}{40,80,180}
\definecolor{spectrumRed}{RGB}{210,55,55}
...
\begin{scope}[shift={(6, 12.10)}]
    % Outer box
    \draw[rounded corners=3pt, genGreen, line width=1pt, fill=genGreenLL]
        (-4.8,-2.00) rectangle (4.8, 4.45);
    % Title
    \node[anchor=north, font=\LARGE\bfseries, text=genGreen] at (0, 4.30) {Generate};
    ...
    \begin{scope}[shift={(-3.4, 3.7)}]
        ...
        \node[cloud, draw=genGreen, line width=0.9pt, fill=white,
        ...
        \node[circle, fill=genGreen, minimum size=2mm, inner sep=0pt] (cd1) at (-0.20, -0.05) {};
        \node[circle, fill=genGreen, minimum size=2mm, inner sep=0pt] (cd2) at ( 0.20, -0.05) {};
        \node[circle, fill=genGreen, minimum size=2mm, inner sep=0pt] (cd3) at ( 0.00,  0.18) {};
        \node[circle, fill=genGreen, minimum size=2mm, inner sep=0pt] (cd4) at ( 0.00, -0.20) {};
        \draw[genGreen, line width=0.6pt] (cd1)--(cd3) (cd2)--(cd3)
                                          (cd1)--(cd4) (cd2)--(cd4)
                                          (cd1)--(cd2);
    \end{scope}

    % Inner sub-box
    \draw[rounded corners=2pt, genGreen!60, thin]
        (-4.5,-1.70) rectangle (4.5, 2.55);
    \node[anchor=north, font=\normalsize, align=center] at (0, 2.40)
...
\begin{scope}[shift={(-0.2, 12.10)}]
    ...
    \end{scope}
    \node[anchor=north, align=center, font=\small] at (0,-0.30)
        {Rule-based\\derivation\\graphs\\(M\O D\\pipeline)};
\end{scope}

% Arrow from MØD to Generate (with label)
\draw[grayA] (1.0, 12.10) -- (1.20, 12.10);
\node[font=\itshape\small, align=center] at (1.6, 10.80) {proxy\\supervision};
...
\begin{scope}[shift={(11.5, 14.0)}]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \draw[genGreen, line width=1pt] (-0.35, 0.00) -- (-0.05, 0.00);
        \draw[genGreen, line width=1pt] (-0.05, 0.00) -- ( 0.20, 0.22);
        \draw[genGreen, line width=1pt] (-0.05, 0.00) -- ( 0.20,-0.22);
        \draw[genGreen, line width=1pt] ( 0.20, 0.22) -- ( 0.20,-0.22);
        % Atoms (small filled green dots with outline)
        \foreach \x/\y in {-0.35/0.00, -0.05/0.00, 0.20/0.22, 0.20/-0.22} {
            \draw[genGreen, line width=0.7pt, fill=genGreen!55]
                (\x,\y) circle (0.075);
        }
    \end{scope}
    ...
    \begin{scope}[shift={(0,-1.7)}]
        \node[circle, fill=genGreen!50, draw=genGreen, minimum size=3mm, inner sep=0pt] (cf1) at (-0.2, 0.10) {};
        \node[circle, fill=genGreen!25, draw=genGreen!70, minimum size=3mm, inner sep=0pt] (cf2) at ( 0.2, 0.05) {};
        \node[circle, fill=genGreen!50, draw=genGreen, minimum size=3mm, inner sep=0pt] (cf3) at (-0.10,-0.30) {};
        \node[circle, fill=white, draw=genGreen!70, minimum size=3mm, inner sep=0pt] (cf4) at ( 0.30,-0.30) {};
        \draw[dashed, genGreen!70, thin] (cf1)--(cf2);
        \draw[dashed, genGreen!70, thin] (cf3)--(cf4);
    \end{scope}
...
\node[font=\itshape\small, align=center] at (8.85, 9.65) {end-to-end\\training};
...
\begin{scope}[shift={(6, 6.0)}]
    % Outer box
    \draw[rounded corners=3pt, scoBlue, line width=1pt, fill=scoBlueLL]
        (-4.8,-2.00) rectangle (4.8, 3.20);
    % Title
    \node[anchor=north, font=\LARGE\bfseries, text=scoBlue] at (0, 3.05) {Score};
    ...
    \begin{scope}[shift={(-3.4, 2.5)}]
        ...
        \draw[scoBlue, thick] (h0)--(h1)--(h2)--(h3)--(h4)--(h5)--cycle;
        \node[circle, fill=scoBlue, minimum size=1.4mm, inner sep=0pt] (m0) at (0,0) {};
        \foreach \i in {0,...,5} {
            \node[circle, fill=scoBlue, minimum size=1.4mm, inner sep=0pt] at (h\i) {};
            \draw[scoBlue, thin, ->, shorten >=0.6mm, shorten <=0.6mm]
                (m0) -- (h\i);
        }
    \end{scope}

    % Three sub-panels: Fragments, Molecule, Spectrum
    \draw[rounded corners=2pt, scoBlue!50, thin]
        (-4.5, 0.20) rectangle (4.5, 2.05);
    % Vertical separators
    \draw[scoBlue!40, thin] (-1.5, 0.20) -- (-1.5, 2.05);
    \draw[scoBlue!40, thin] ( 1.5, 0.20) -- ( 1.5, 2.05);
    ...
    \node[circle, fill=atomBlue, draw=atomBlueE, minimum size=3mm, inner sep=0pt] at (-3.7, 1.05) {};
    \node[circle, fill=atomBlue!70, draw=atomBlueE, minimum size=3mm, inner sep=0pt] at (-3.2, 1.10) {};
    \node[circle, fill=atomBlue!90, draw=atomBlueE, minimum size=3mm, inner sep=0pt] at (-2.7, 1.05) {};
    \node[circle, fill=atomBlue!60, draw=atomBlueE, minimum size=3mm, inner sep=0pt] at (-2.2, 1.10) {};
    \node[font=\small] at (-1.75, 1.05) {$\dots$};

    % Molecule: small graph
    \begin{scope}[shift={(0, 1.05)}]
        \node[circle, fill=atomDark, draw=atomDark, minimum size=3mm, inner sep=0pt] (mm1) at (-0.40, 0.10) {};
        \node[circle, fill=atomBlue, draw=atomBlueE, minimum size=3mm, inner sep=0pt] (mm2) at (-0.05, 0.20) {};
        \node[circle, fill=atomDark, draw=atomDark, minimum size=3mm, inner sep=0pt] (mm3) at ( 0.30, 0.05) {};
        \node[circle, fill=atomBlue, draw=atomBlueE, minimum size=3mm, inner sep=0pt] (mm4) at (-0.20,-0.25) {};
        \node[circle, fill=white, draw=atomBlueE, minimum size=3mm, inner sep=0pt] (mm5) at ( 0.45,-0.30) {};
    ...
    \begin{scope}[shift={(1.4,-0.90)}]
        ...
        \foreach \px/\py in {
            ...
            \node[circle, fill=scoBlue!70, draw=scoBlue, minimum size=1.4mm, inner sep=0pt] at (\px, \py) {};
        }
    \end{scope}

    % Arrow from Molecule to Shared latent space
    \draw[->, scoBlue, thick] (0, 0.50) -- (0, 0.10);
\end{scope}
...
\begin{scope}[shift={(2.0, 2.55)}]
    ...
    \begin{scope}[shift={(-2.0, -0.65)}]
        \draw[scoBlue!70, thin] (0,0) -- (4.1, 0);
...
\begin{scope}[shift={(9.4, 2.55)}]
    ...
    \begin{scope}[shift={(-2.4, -0.65)}]
        \draw[spectrumRed!70, thin] (0,0) -- (1.85, 0);
        \foreach \x/\h in {0.10/0.45, 0.25/0.85, 0.40/0.30, 0.55/0.65, 0.70/0.95,
                          0.90/0.40, 1.05/0.55, 1.20/0.20, 1.35/0.70, 1.50/0.30,
                          1.65/0.50, 1.78/0.25} {
            \draw[spectrumRed, line width=0.9pt] (\x, 0) -- (\x, \h);
        }
    \end{scope}
    % Double-ended arrow
    \draw[<->, line width=1pt] (-0.30, -0.20) -- (0.30, -0.20);
    % Two small molecules + dots
    \begin{scope}[shift={(0.85,-0.30)}]
        \node[circle, fill=atomBlueE, minimum size=2mm, inner sep=0pt] (im1) at (0,0.18) {};
        \node[circle, fill=white, draw=atomBlueE, minimum size=2mm, inner sep=0pt] (im2) at (-0.20,-0.05) {};
        \node[circle, fill=atomBlue, draw=atomBlueE, minimum size=2mm, inner sep=0pt] (im3) at (0.20,-0.05) {};
        \node[circle, fill=atomDark, minimum size=2mm, inner sep=0pt] (im4) at (0.05,-0.30) {};
        \draw[thin] (im1)--(im2) (im1)--(im3) (im3)--(im4) (im2)--(im4);
    \end{scope}
    \begin{scope}[shift={(1.65,-0.30)}]
        \node[circle, fill=atomBlue, draw=atomBlueE, minimum size=2mm, inner sep=0pt] (jm1) at (0,0.18) {};
        \node[circle, fill=atomBlueE, minimum size=2mm, inner sep=0pt] (jm2) at (-0.18,-0.10) {};
        \node[circle, fill=white, draw=atomBlueE, minimum size=2mm, inner sep=0pt] (jm3) at (0.20,-0.05) {};
        \node[circle, fill=atomBlue, draw=atomBlueE, minimum size=2mm, inner sep=0pt] (jm4) at (0.05,-0.32) {};
