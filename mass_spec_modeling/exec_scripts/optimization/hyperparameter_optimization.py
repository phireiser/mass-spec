...
to find the best configuration for the fragment-aided bidirectional model.
...
def train_with_config(config: Dict[str, Any], epochs_fwd: int, epochs_bwd: int, device: str = "cuda") -> Dict[str, float]:
    """
...
def objective(trial: "optuna.Trial", epochs_fwd: int, epochs_bwd: int, device: str) -> float:
    """
    ...
    metrics = train_with_config(config, epochs_fwd, epochs_bwd, device)
...
def run_optimization(n_trials: int, epochs_fwd: int, epochs_bwd: int, device: str = "cuda"):
    ...
    # Run optimization
    study.optimize(
        lambda trial: objective(trial, epochs_fwd, epochs_bwd, device),
        n_trials=n_trials,
    ...
    # Save to JSON
    output_file = Path("best_hyperparams.json")
